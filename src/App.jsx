      ...
      Component: ({ onDone, modoMayor }) => {
        ...
        return (
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, maxWidth: 340, margin: "0 auto 16px" }}>
              {g.items.map((item, i) => (
      ...
      Component: ({ onDone, modoMayor }) => {
        ...
        const genOps = () => {
          ...
          for (let i = 0; i < 8; i++) {
            ...
            let a, b, ans, sym;
            if (tipo === 0) { a = Math.floor(Math.random()*50)+10; b = Math.floor(Math.random()*50)+10; ans = a+b; sym = "+"; }
            else if (tipo === 1) { a = Math.floor(Math.random()*50)+30; b = Math.floor(Math.random()*30)+5; ans = a-b; sym = "-"; }
            else if (tipo === 2) { a = Math.floor(Math.random()*9)+2; b = Math.floor(Math.random()*9)+2; ans = a*b; sym = "x"; }
            else { b = [2,3,4,5,6,10][Math.floor(Math.random()*6)]; a = b*(Math.floor(Math.random()*9)+2); ans = a/b; sym = "/"; }
      ...
      Component: ({ onDone, modoMayor }) => {
        ...
        const [queue] = useState(() => {
          ...
          for (let i = 0; i < RONDAS; i++) {
            if (i > 0 && Math.random() < 0.35) regla = regla === "color" ? "forma" : "color";
  ...
  comprension: [
    {
      ...
      Component: ({ onDone, modoMayor }) => {
        const M = modoMayor || false;
        const ITEMS = shuffle([
          { contexto: "Al llegar a casa encontro la puerta abierta y las luces encendidas, aunque vivia sola.", pregunta: "Que es mas probable?", opts: ["Dejo la puerta abierta sin recordarlo","Entro alguien","La puerta se abrio sola","Las luces se encendieron solas"], ans: "Dejo la puerta abierta sin recordarlo" },
          { contexto: "Carlos siempre llega 10 minutos antes. Hoy son las 9:15 y la reunion era a las 9:00, no ha llegado.", pregunta: "Que es lo mas probable?", opts: ["Carlos decidio no ir","Algo inesperado le ocurrio","Carlos cambio su costumbre","La reunion fue cancelada"], ans: "Algo inesperado le ocurrio" },
          { contexto: "El medico dijo que sus examenes eran perfectos y que podia seguir con su dieta actual.", pregunta: "Que podemos concluir?", opts: ["Tiene enfermedad oculta","La dieta es saludable para el","El medico no reviso bien","Debe cambiar la dieta"], ans: "La dieta es saludable para el" },
          { contexto: "Ana ahorro 2 anos para un carro. Esta semana firmo los papeles del nuevo carro.", pregunta: "Que podemos inferir?", opts: ["Ana se endeudo","Compro el carro con sus ahorros","El carro fue un regalo","Pidio un prestamo"], ans: "Compro el carro con sus ahorros" },
        ]).slice(0, 4);
        const [cur, setCur] = useState(0);
        const [score, setScore] = useState(0);
        const [sel, setSel] = useState(null);
